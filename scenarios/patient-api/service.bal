...
configurable string baseURL = "https://hapi.fhir.org/baseR4";
...
final fhir:FHIRConnector fhirConnector = check new (fhirServerConfig);
...
service / on new fhirr4:Listener(9090, apiConfig) {
    ...
    isolated resource function get fhir/r4/Patient/[string id](r4:FHIRContext fhirContext) returns Patient|r4:OperationOutcome|r4:FHIRError {
        fhir:FHIRResponse|fhir:FHIRError response = fhirConnector->getById("Patient", id);
        if response is fhir:FHIRResponse {
            log:printInfo("Response from FHIR server: ", statusCode = response.httpStatusCode, response = response.'resource);
            Patient|error patientData = response.'resource.cloneWithType(Patient);
            if patientData is Patient {
                return patientData;
            }
        }
        return r4:createFHIRError("Error occurred while retrieving the patient data", r4:ERROR, r4:INFORMATIONAL, httpStatusCode = http:STATUS_INTERNAL_SERVER_ERROR);
    }
    ...
    isolated resource function get fhir/r4/Patient(r4:FHIRContext fhirContext) returns r4:Bundle|r4:OperationOutcome|error {
        ...
        foreach var requestSearchParameter in requestSearchParameters {
            searchParameters[requestSearchParameter[0].name] = [requestSearchParameter[0].value];
        }
        if searchParameters != {} {
            fhir:FHIRResponse|fhir:FHIRError response = fhirConnector->search("Patient", searchParameters);
            if response is fhir:FHIRResponse {
                log:printInfo("Response from FHIR server: ", statusCode = response.httpStatusCode, response = response.'resource);
                return check response.'resource.cloneWithType(r4:Bundle);
            }
        }
        return r4:createFHIRError("Not supported", r4:ERROR, r4:INFORMATIONAL, httpStatusCode = http:STATUS_UNPROCESSABLE_ENTITY);
    }
